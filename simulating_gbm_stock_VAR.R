...
AMZN_log_returns <- dailyReturn(Ad(AMZN), type="log")
...
for (i in 1:n_sims) {
  values <-  gbm_sim(periodos = n_periods, close_prices = amazon_adj, vector_returns = AMZN_log_returns)
  sims[, i] <- values
}
sims[, n_sims+1] <- 1:n_periods

# Seteo del plot
plot(sims[, n_sims+1], sims[,1], type="l", xlab="tiempo", ylab="W(t)", ylim=c(800, 7000))
...
serie <- as.numeric(amazon_adj[2668:2919])
lines(sims[, n_sims+1], serie, type="l", col="black")
...
# Luego, tendremos n_sims X (n_periods-1) = 500 X 251 125,500 rentabilidades
# Histograma de todas esas rentabilidades
hist(returns_gbm, 100)

# Porcentil 5 -- VAR al  99% y 95%
quantile(returns_gbm, c(0.01, 0.05), na.rm=TRUE) 

1%            5% 
-0.05064950 -0.03596166    :   -5.06%  Y -3.6%
...
# Ahora seleccionamos por ejemplo, el ultimo periodo simulado, y obtenemos su distribucion
# y estadisticos.
